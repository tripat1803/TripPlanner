            ...
            <div className='absolute top-0 left-0 w-full h-full flex items-end p-4 z-[1]'>
                <div className='w-full text-white flex flex-col gap-[2px]'>
                    ...
                    <div className='w-full flex justify-between items-center gap-3'>
                        ...
                        <p className='text-sm flex items-center gap-2'><BsCalendar4Week/> <span>{data.goingDate}</span></p>
                    </div>
                </div>
            </div>
        </div>
...
export default function ListingPage() {
    ...
    return (alltrip.length > 0) && (
        <div className='flex justify-center items-center py-16 px-8 sm:px-6'>
            <div className='w-full max-w-screen-xl flex flex-col gap-12'>
                <div className='flex items-center gap-3 justify-between'>
                    <h2 className='text-4xl font-semibold'>Your Trips</h2>
                    <button className='py-1 px-3 bg-[linear-gradient(90deg,#10B5CB_0%,#0073A8_100%)] text-white rounded-lg'>Create new</button>
                </div>
